import React, { useState } from 'react';
import Navbar from './Navbar';
import Footer from './Footer';
import { useAuth } from './AuthContext';

const sections = [
    {
        title: 'Full Nigerian Breakfast',
        items: [
            { name: 'Boiled or Fried Yam', price: '₦5,500', description: 'With sauce of your choice and steamed veggies.', tag: null },
            { name: 'Boiled or Fried Plantain', price: '₦5,500', description: 'With sauce of your choice and steamed veggies.', tag: null },
        ],
    },
    {
        title: 'Egg & Sauces',
        items: [
            { name: 'Nigerian Egg Sauce', price: '₦3,300', description: '', tag: 'Popular' },
            { name: 'Plain Fried Eggs', price: '₦1,500', description: '', tag: null },
            { name: 'Boiled Eggs', price: '₦600', description: '', tag: null },
            { name: 'Sunny-Side-Up Egg', price: '₦1,500', description: '', tag: null },
            { name: 'Poached Egg', price: '₦1,500', description: '', tag: null },
            { name: 'Avocado Toast', price: '₦13,000', description: '', tag: 'Premium' },
        ],
    },
];

export default function Breakfast() {
    const { user, logout } = useAuth();
    const [search, setSearch] = useState('');

    const handleOrder = () => {
        if (!user) { sessionStorage.setItem('olive_intended', '/place-order'); window.location.href = '/login'; return; }
        window.location.href = '/place-order';
    };

    const filtered = sections.map(s => ({
        ...s,
        items: s.items.filter(m => m.name.toLowerCase().includes(search.toLowerCase())),
    })).filter(s => s.items.length > 0);

    return (
        <div className="main-meals-page">
            <Navbar user={user} onLogout={logout} />

            <div className="mm-hero">
                <div className="mm-hero-overlay" />
                <div className="mm-hero-content">
                    <span className="mm-hero-eyebrow">EXPLORE</span>
                    <h1>Breakfast</h1>
                    <p>Fresh, energizing breakfasts to start your day right.</p>
                    <div className="mm-search-wrap">
                        <svg className="mm-search-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg>
                        <input
                            className="mm-search"
                            type="text"
                            placeholder="Search breakfast…"
                            value={search}
                            onChange={e => setSearch(e.target.value)}
                        />
                        {search && <button className="mm-search-clear" onClick={() => setSearch('')}>✕</button>}
                    </div>
                </div>
            </div>

            <div className="mm-sections">
                {filtered.length === 0 ? (
                    <div className="mm-empty">No items match your search.</div>
                ) : (
                    filtered.map((section, si) => (
                        <div key={si} className="mm-section">
                            <div className="mm-section-header">
                                <h2>{section.title}</h2>
                                <div className="mm-section-line" />
                            </div>
                            <div className="mm-list">
                                {section.items.map((meal, i) => (
                                    <div key={i} className="mm-row">
                                        <div className="mm-row-info">
                                            <div className="mm-row-name">
                                                {meal.name}
                                                {meal.tag && <span className="mm-tag">{meal.tag}</span>}
                                            </div>
                                            {meal.description && <p className="mm-row-desc">{meal.description}</p>}
                                        </div>
                                        <div className="mm-row-actions">
                                            <span className="mm-price">{meal.price}</span>
                                        </div>
                                    </div>
                                ))}
                                <div className="mm-group-footer">
                                    <p className="mm-group-hint">Select your item and place your order.</p>
                                    <button className="mm-order-btn mm-order-btn-group" onClick={handleOrder}>
                                        Place Order
                                    </button>
                                </div>
                            </div>
                        </div>
                    ))
                )}
            </div>

            <Footer />
        </div>
    );
}
